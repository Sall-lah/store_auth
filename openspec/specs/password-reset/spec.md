...
## Purpose
TBD - User password recovery workflow via 6-digit numeric OTP code.
...
The system SHALL allow users to set a new password after successfully verifying the OTP code from the password reset flow. The new password SHALL be hashed using bcrypt (cost factor 12) before storage. Upon successful password reset, the system SHALL mark the OTP code as used, nullify/clear the raw OTP code from persistence, invalidate remaining active password reset OTPs for that user, and update the user's password.

#### Scenario: Successful password reset
- **WHEN** a user submits a valid OTP code and a new password to `POST /api/auth/reset-password`
- **THEN** the system verifies the OTP, hashes the new password with bcrypt, updates the user record, marks the OTP as used and scrubs the raw code, invalidates any remaining reset OTPs, and returns HTTP 200
